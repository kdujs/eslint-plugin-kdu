...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      category: 'essential',
      url: 'https://github.com/kdujs/eslint-plugin-kdu/blob/v5.0.0/docs/rules/valid-k-on.md'
    },
    fixable: null,
    schema: [
      {
  ...
  create (context) {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      "KAttribute[directive=true][key.name='on']" (node) {
        for (const modifier of node.key.modifiers) {
          if (!isValidModifier(modifier, customModifiers)) {
            context.report({
              node,
              loc: node.loc,
              message: "'k-on' directives don't support the modifier '{{modifier}}'.",
              data: { modifier }
            })
          }
        }
        ...
        if (
          ...
          if (node.value && sourceCode.getText(node.value.expression)) {
            ...
            context.report({
              node,
              loc: node.loc,
              message: 'Avoid using JavaScript keyword as "k-on" value: {{value}}.',
              data: { value }
            })
          } else {
            context.report({
              node,
              loc: node.loc,
              message: "'k-on' directives require a value or verb modifier (like 'stop' or 'prevent')."
            })
          }
        }
      }
