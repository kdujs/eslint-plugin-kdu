...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://kdujs-eslint.web.app/rules/no-static-inline-styles.html'
    },
    fixable: null,
    schema: [
      {
  ...
  create(context) {
     ...
     *    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     * @param {KDirective} node `:style` node to check
     * @returns {Property[] | [KDirective]} the static properties.
     */
     ...
     * @param {KDirective} node `:style` node to check
     */
    function verifyKBindStyle(node) {
    ...
    const visitor = {
      ...
      "KAttribute[directive=false][key.name='style']"(node) {
        context.report({
          node,
    ...
    if (!context.options[0] || !context.options[0].allowBinding) {
      visitor[
        "KAttribute[directive=true][key.name.name='bind'][key.argument.name='style']"
      ] = verifyKBindStyle
    }
