...
/**
 * Check whether the given attribute is `k-model` directive.
 * @param {KAttribute | KDirective | undefined | null} node
 * @returns { node is KDirective }
 */
function isKModel(node) {
...
/**
 * Check whether the given attribute is plain attribute, `k-bind` directive or `k-model` directive.
 * @param {KAttribute | KDirective | undefined | null} node
 * @returns { node is KAttribute }
 */
function isKAttributeOrKBindOrKModel(node) {
  return isKAttribute(node) || isKBind(node) || isKModel(node)
}
...
function create(context) {
    ...
    context.report({
      ...
      fix(fixer) {
        ...
        if (isKBindObject(node)) {
          ...
          isMoveUp = isKAttributeOrKBindOrKModel
        } else if (isKAttributeOrKBindOrKModel(node)) {
  ...
  function getAttributeAndPositionList(node) {
    const attributes = node.attributes.filter((node, index, attributes) => {
      if (
        isKBindObject(node) &&
        (isKAttributeOrKBindOrKModel(attributes[index - 1]) ||
          isKAttributeOrKBindOrKModel(attributes[index + 1]))
      ) {
        // In Kdu 3, ignore `k-bind="object"`, which is
        // a pair of `k-bind:foo="..."` and `k-bind="object"` and
        // a pair of `k-model="..."` and `k-bind="object"`,
    ...
    function getPositionFromAttrIndex(index) {
      ...
      if (isKBindObject(node)) {
        ...
        // In Kdu 3, if change the order of `k-bind:foo="..."`, `k-model="..."` and `k-bind="object"`,
        ...
        for (let nextIndex = index + 1; nextIndex < len; nextIndex++) {
          ...
          if (isKAttributeOrKBindOrKModel(next) && !isKBindObject(next)) {
