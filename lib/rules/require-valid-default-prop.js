...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://kdujs-eslint.web.app/rules/require-valid-default-prop.html'
    },
    fixable: null,
    schema: []
  },
  ...
  create(context) {
     ...
     */
    const kduObjectPropsContexts = new Map()
    /**
    ...
    return utils.compositingVisitors(
      ...
      utils.defineKduVisitor(context, {
        onKduObjectEnter(obj) {
          ...
          kduObjectPropsContexts.set(obj, propContexts)
        },
        /**
         * @param {FunctionExpression | FunctionDeclaration | ArrowFunctionExpression} node
         * @param {KduObjectData} data
         */
        ':function'(node, { node: kduNode }) {
          const data = kduObjectPropsContexts.get(kduNode)
          if (!data || !scopeStack) {
            return
          }
        ...
        onKduObjectExit(obj) {
          const data = kduObjectPropsContexts.get(obj)
          if (!data) {
            return
          }
