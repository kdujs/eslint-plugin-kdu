 ...
 */
function createForKduSyntax(context) {
  ...
  /**
   * Checks if the given node is CSS k-bind() without quote.
   * @param {KExpressionContainer} node
   * @param {Expression} expression
   */
  function isStyleVariableWithoutQuote(node, expression) {
    const styleVars = getStyleVariablesContext(context)
    if (!styleVars || !styleVars.kBinds.includes(node)) {
      return false
    }

    const kBindToken = tokenStore.getFirstToken(node)
    const tokens = tokenStore.getTokensBetween(kBindToken, expression)

    return tokens.every(isLeftParen)
  }
  /**
   * @param {KExpressionContainer & { expression: Expression | KFilterSequenceExpression | null }} node
   */
  function verify(node) {
    ...
    const expression =
      node.expression.type === 'KFilterSequenceExpression'
  ...
  return {
    "KAttribute[directive=true][key.name.name='bind'] > KExpressionContainer":
      verify,
    'KElement > KExpressionContainer': verify
  }
}
