...
module.exports = {
  meta: {
    ...
    docs: {
      description: 'disallow irregular whitespace in `.kdu` files',
      categories: undefined,
      url: 'https://kdujs-eslint.web.app/rules/no-irregular-whitespace.html',
      extensionRule: true,
  ...
  create(context) {
     ...
     * @param {KLiteral} node to check for matching errors.
     * @returns {void}
     * @private
     */
     ...
     * @param {KText} node to check for matching errors.
     * @returns {void}
     * @private
     */
    ...
    const bodyVisitor = utils.defineTemplateBodyVisitor(context, {
      ...(skipHTMLAttributeValues
        ? {
            'KAttribute[directive=false] > KLiteral':
              removeInvalidNodeErrorsInHTMLAttributeValue
          }
        : {}),
      ...(skipHTMLTextContents
        ? { KText: removeInvalidNodeErrorsInHTMLTextContent }
        : {}),
