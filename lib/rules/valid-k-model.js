...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      category: 'essential',
      url: 'https://github.com/kdujs/eslint-plugin-kdu/blob/v5.0.0/docs/rules/valid-k-model.md'
    },
    fixable: null,
    schema: []
  },
  ...
  create (context) {
    return utils.defineTemplateBodyVisitor(context, {
      "KAttribute[directive=true][key.name='model']" (node) {
        ...
        if (!isValidElement(element)) {
          context.report({
            node,
            loc: node.loc,
            message: "'k-model' directives aren't supported on <{{name}}> elements.",
            data: { name }
          })
        }
        ...
        if (name === 'input' && utils.hasAttribute(element, 'type', 'file')) {
          context.report({
            node,
            loc: node.loc,
            message: "'k-model' directives don't support 'file' input type."
          })
        }
        ...
        if (node.key.argument) {
          context.report({
            node,
            loc: node.loc,
            message: "'k-model' directives require no argument."
          })
        }
        ...
        for (const modifier of node.key.modifiers) {
          if (!VALID_MODIFIERS.has(modifier)) {
            context.report({
              node,
              loc: node.loc,
              message: "'k-model' directives don't support the modifier '{{name}}'.",
              data: { name: modifier }
            })
          }
        }
        if (!utils.hasAttributeValue(node)) {
          context.report({
            node,
            loc: node.loc,
            message: "'k-model' directives require that attribute value."
          })
        }
        if (node.value) {
          if (!isLhs(node.value.expression)) {
            context.report({
              node,
              loc: node.loc,
              message: "'k-model' directives require the attribute value which is valid as LHS."
            })
          }
          ...
          for (const reference of node.value.references) {
            ...
            if (variable != null) {
              context.report({
                node,
                loc: node.loc,
                message: "'k-model' directives cannot update the iteration variable '{{varName}}' itself.",
