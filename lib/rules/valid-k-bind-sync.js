/**
 * @fileoverview enforce valid `.sync` modifier on `k-bind` directives
 * @author NKDuy
 */
'use strict'
...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'enforce valid `.sync` modifier on `k-bind` directives',
      categories: ['essential'],
      url: 'https://kdujs-eslint.web.app/rules/valid-k-bind-sync.html'
    },
    fixable: null,
    schema: [],
    messages: {
      unexpectedInvalidElement:
        "'.sync' modifiers aren't supported on <{{name}}> non Kdu-components.",
      unexpectedOptionalChaining:
        "Optional chaining cannot appear in 'k-bind' with '.sync' modifiers.",
