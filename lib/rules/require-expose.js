/**
 * @fileoverview Require `expose` in Kdu components
 * @author NKDuy
 */
'use strict'
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://kdujs-eslint.web.app/rules/require-expose.html'
    },
    fixable: null,
    schema: [],
    messages: {
  ...
  create(context) {
    ...
    return utils.defineKduVisitor(context, {
      onSetupFunctionEnter(node, { node: kduNode }) {
        setupFunctions.set(node, kduNode)
      ...
      ':function:exit'(node) {
        if (scopeStack && scopeStack.returnFunction) {
          const kduNode = setupFunctions.get(node)
          if (kduNode) {
            setupRender.add(kduNode)
          }
        }
        scopeStack = scopeStack && scopeStack.upper
      },
      onKduObjectExit(component, { type }) {
