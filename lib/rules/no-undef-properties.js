...
const reserved = require('../utils/kdu-reserved.json')
...
const GROUP_ASYNC_DATA = 'asyncData' // Dorayaki.js
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: 'https://kdujs-eslint.web.app/rules/no-undef-properties.html'
    },
    fixable: null,
    schema: [
      {
  ...
  create(context) {
    ...
    /** Kdu component context */
    class KduComponentContext {
      constructor() {
    ...
    /** @type {Map<ASTNode, KduComponentContext>} */
    const kduComponentContextMap = new Map()

    /**
     * @param {ASTNode} node
     * @returns {KduComponentContext}
     */
    function getKduComponentContext(node) {
      let ctx = kduComponentContextMap.get(node)
      if (!ctx) {
        ctx = new KduComponentContext()
        kduComponentContextMap.set(node, ctx)
      }
      return ctx
    }
    /**
     * @returns {KduComponentContext|void}
     */
    function getKduComponentContextForTemplate() {
      const keys = [...kduComponentContextMap.keys()]
      const exported =
        keys.find(isScriptSetupProgram) || keys.find(utils.isInExportDefault)
      return exported && kduComponentContextMap.get(exported)
    ...
    const scriptVisitor = utils.compositingVisitors(
      {
        Program() {
          ...
          const ctx = getKduComponentContext(programNode)
      ...
      utils.defineKduVisitor(context, {
        onKduObjectEnter(node) {
          const ctx = getKduComponentContext(node)
        ...
        'ObjectExpression > Property > :function[params.length>0]'(
          ...
          if (property.parent === kduData.node) {
            ...
            if (parentProperty.parent === kduData.node) {
              ...
              if (parentParentProperty.parent === kduData.node) {
                if (
          ...
          const ctx = getKduComponentContext(kduData.node)
          ctx.verifyReferences(propertyReferences, { props })
        },
        onSetupFunctionEnter(node, kduData) {
          const propertyReferences =
            propertyReferenceExtractor.extractFromFunctionParam(node, 0)
          const ctx = getKduComponentContext(kduData.node)
          ctx.verifyReferences(propertyReferences, {
        ...
        onRenderFunctionEnter(node, kduData) {
          const ctx = getKduComponentContext(kduData.node)

          // Check for Kdu 3.x render
          const propertyReferences =
            propertyReferenceExtractor.extractFromFunctionParam(node, 0)
          ctx.verifyReferences(propertyReferences)

          if (kduData.functional) {
            // Check for Kdu 2.x render & functional
         ...
         * @param {KduObjectData} kduData
         */
        'ThisExpression, Identifier'(node, kduData) {
      ...
      {
        'Program:exit'() {
          const ctx = getKduComponentContextForTemplate()
          if (!ctx) {
            return
          }
    ...
    const templateVisitor = {
      ...
      KExpressionContainer(node) {
        ...
        ctx.verifyReferences(
          propertyReferenceExtractor.extractFromKExpressionContainer(node, {
            ignoreGlobals: true
          })
        )
      }
