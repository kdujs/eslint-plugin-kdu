...
const kduComponentOptions = require('../utils/kdu-component-options.json')
// ------------------------------------------------------------------------------
// Rule Definition
// ------------------------------------------------------------------------------
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      recommended: false,
      url: 'https://kdujs-eslint.web.app/rules/no-potential-component-option-typo.html'
    },
    fixable: null,
    schema: [
      {
        ...
        properties: {
          presets: {
            ...
            items: {
              type: 'string',
              enum: ['all', 'kdu', 'kdu-router', 'dorayaki']
            },
            uniqueItems: true,
            minItems: 0
          },
  ...
  create(context) {
    ...
    /** @type {('all' | 'kdu' | 'kdu-router' | 'dorayaki')[]} */
    const presets = option.presets || ['kdu']
    const threshold = option.threshold || 1
    /** @type {Set<string>} */
    const candidateOptionSet = new Set(custom)
    for (const preset of presets) {
      if (preset === 'all') {
        for (const opts of Object.values(kduComponentOptions)) {
          for (const opt of opts) {
            candidateOptionSet.add(opt)
          }
        }
      } else {
        for (const opt of kduComponentOptions[preset]) {
          candidateOptionSet.add(opt)
        }
      }
    }
