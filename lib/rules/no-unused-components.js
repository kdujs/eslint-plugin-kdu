...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      category: 'essential',
      url: 'https://github.com/kdujs/eslint-plugin-kdu/blob/v5.0.0/docs/rules/no-unused-components.md'
    },
    fixable: null,
    schema: [{
      type: 'object',
  ...
  create (context) {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      ...
      "KAttribute[directive=true][key.name='bind'][key.argument='is']" (node) {
        if (node.value.type !== 'KExpressionContainer') return
      ...
      "KElement[name='template']" (rootNode) {
        templateLocation = templateLocation || rootNode.loc.start
      },
      "KElement[name='template']:exit" (rootNode) {
        if (
