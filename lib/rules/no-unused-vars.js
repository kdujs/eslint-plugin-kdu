...
/**
 * @typedef {KVariable['kind']} VariableKind
 */
 ...
 * @param {KElement} node The element node
 * @returns { { [kind in VariableKind]?: KVariable[] } } The variables of grouped by directive kind.
 */
function groupingVariables(node) {
  /** @type { { [kind in VariableKind]?: KVariable[] } } */
  const result = {}
 ...
 * @param {KVariable} variable the given variable to check
...
function isDestructuringVar(variable) {
  ...
  while (parent) {
    if (
      parent.type === 'KForExpression' ||
      parent.type === 'KSlotScopeExpression'
    ) {
      return false
    }
...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'disallow unused variable definitions of k-for directives or scope attributes',
      categories: ['kdu3-essential', 'essential'],
      url: 'https://kdujs-eslint.web.app/rules/no-unused-vars.html'
    },
    fixable: null,
    schema: [
      {
