...
module.exports = {
  meta: {
    type: 'problem',
    docs: {
      description: 'disallow specific classes in Kdu components',
      url: 'https://kdujs-eslint.web.app/rules/no-restricted-class.html',
  ...
  create(context) {
    ...
    return utils.defineTemplateBodyVisitor(context, {
      /**
       * @param {KAttribute & { value: KLiteral } } node
       */
      ...
      "KAttribute[directive=true][key.name.name='bind'][key.argument.name='class'] > KExpressionContainer.value"(
        node
      ) {
        if (!node.expression) {
          return
