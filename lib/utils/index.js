 ...
 */
/**
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentArrayProp} ComponentArrayProp
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentObjectProp} ComponentObjectProp
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentTypeProp} ComponentTypeProp
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentUnknownProp} ComponentUnknownProp
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentProp} ComponentProp
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentArrayEmit} ComponentArrayEmit
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentObjectEmit} ComponentObjectEmit
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentTypeEmit} ComponentTypeEmit
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentUnknownEmit} ComponentUnknownEmit
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ComponentEmit} ComponentEmit
 */
/**
 ...
 */
/**
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').KduObjectType} KduObjectType
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').KduObjectData} KduObjectData
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').KduVisitor} KduVisitor
 * @typedef {import('../../typings/eslint-plugin-kdu/util-types/utils').ScriptSetupVisitor} ScriptSetupVisitor
 */
...
const KDU2_BUILTIN_COMPONENT_NAMES = new Set(
  require('./kdu2-builtin-components')
)
const KDU3_BUILTIN_COMPONENT_NAMES = new Set(
  require('./kdu3-builtin-components')
)
const path = require('path')
const kduEslintParser = require('kdu-eslint-parser')
const { traverseNodes, getFallbackKeys } = kduEslintParser.AST
...
function wrapContextToOverrideReportMethodToSkipDynamicArgument(context) {
  ...
  const traverseNodes = kduEslintParser.AST.traverseNodes
  traverseNodes(templateBody, {
    enterNode(node, parent) {
      if (
        parent &&
        parent.type === 'KDirectiveKey' &&
        node.type === 'KExpressionContainer'
      ) {
...
module.exports = {
   ...
   * If the parser service of `kdu-eslint-parser` was not found,
   ...
   */
  /**
   * Wrap a given core rule to apply it to Kdu.js template.
  ...
  wrapCoreRule(coreRuleName, options) {
    ...
    if (!coreRule) {
      return {
        meta: {
          type: 'problem',
          docs: {
            url: `https://kdujs-eslint.web.app/rules/${coreRuleName}.html`
          }
        },
        create(context) {
          return defineTemplateBodyVisitor(context, {
            "KElement[name='template'][parent.type='KDocumentFragment']"(node) {
              context.report({
                node,
    ...
    return {
      create(context) {
        ...
        // Move `Program` handlers to `KElement[parent.type!='KElement']`
        if (handlers.Program) {
          handlers[
            applyDocument
              ? 'KDocumentFragment'
              : "KElement[parent.type!='KElement']"
        ...
        if (skipDynamicArguments) {
          ...
          handlers['KDirectiveKey > KExpressionContainer'] = () => {
            withinDynamicArguments = true
          }
          handlers['KDirectiveKey > KExpressionContainer:exit'] = () => {
      ...
      meta: Object.assign({}, coreRule.meta, {
        docs: Object.assign({}, coreRule.meta.docs, {
          ...
          category: null,
          categories,
          url: `https://kdujs-eslint.web.app/rules/${path.basename(
   ...
   * @param {KElement} node The element node to get the previous sibling element.
   * @returns {KElement|null} The previous sibling element.
   */
  prevSibling(node) {
   ...
   * @param {KDirective} node The directive attribute node to check.
   * @param {RuleContext} context The rule context to use parser services.
   ...
   * @returns {KAttribute | null} The found attribute.
   */
  getAttribute,
   ...
   * @param {KElement | KStartTag} node The start tag node to check.
   ...
   * @returns {KDirective | null} The found directive.
   */
  getDirective,
  ...
  /**
   * Returns a generator with all child element k-if chains of the given element.
   * @param {KElement} node The element node to check.
   * @returns {IterableIterator<KElement[]>}
   */
  *iterateChildElementsChains(node) {
    ...
    for (const childNode of node.children) {
      ...
      } else if (childNode.type !== 'KText' || childNode.value.trim() !== '') {
        kIf = false
      }
    }
    if (elementChain.length) {
   ...
   * @param {KElement} node The node to check.
  ...
  isSvgElementNode(node) {
    return node.namespace === kduEslintParser.AST.NS.SVG
  },
  ...
  isMathMLElementNode(node) {
    return node.namespace === kduEslintParser.AST.NS.MathML
  },
  ...
  /**
   * Check whether the given name is Kdu builtin component name or not.
   * @param {string} name The name to check.
   * @returns {boolean} `true` if the name is a builtin component name
   */
  isBuiltInComponentName(name) {
    return (
      KDU3_BUILTIN_COMPONENT_NAMES.has(name) ||
      KDU2_BUILTIN_COMPONENT_NAMES.has(name)
    )
  },

  /**
   * Check whether the given name is Kdu builtin directive name or not.
   * @param {string} name The name to check.
   ...
   * @param {RuleContext} context The ESLint rule context object.
   * @returns {KElement | null} the element of `<script setup>`
   */
  getScriptSetupElement,

  /**
   * Check if current file is a Kdu instance or component and call callback
   * @param {RuleContext} context The ESLint rule context object.
   * @param { (node: ObjectExpression, type: KduObjectType) => void } cb Callback function
   */
  executeOnKdu(context, cb) {
    return compositingVisitors(
      this.executeOnKduComponent(context, cb),
      this.executeOnKduInstance(context, cb)
    )
  },

  /**
   * Define handlers to traverse the Kdu Objects.
   * Some special events are available to visitor.
   *
   * - `onKduObjectEnter` ... Event when Kdu Object is found.
   * - `onKduObjectExit` ... Event when Kdu Object visit ends.
   * - `onSetupFunctionEnter` ... Event when setup function found.
   * - `onRenderFunctionEnter` ... Event when render function found.
   *
   * @param {RuleContext} context The ESLint rule context object.
   * @param {KduVisitor} visitor The visitor to traverse the Kdu Objects.
   */
  defineKduVisitor(context, visitor) {
    /** @type {KduObjectData | null} */
    let kduStack = null
    ...
    function callVisitor(key, node) {
      if (visitor[key] && kduStack) {
        // @ts-expect-error
        visitor[key](node, kduStack)
      }
    }
    ...
    for (const key in visitor) {
      kduVisitor[key] = (node) => callVisitor(key, node)
    }

    /**
     * @param {ObjectExpression} node
     */
    kduVisitor.ObjectExpression = (node) => {
      const type = getKduObjectType(context, node)
      if (type) {
        kduStack = {
          node,
          type,
        ...
        callVisitor('onKduObjectEnter', node)
      }
      callVisitor('ObjectExpression', node)
    }
    kduVisitor['ObjectExpression:exit'] = (node) => {
      callVisitor('ObjectExpression:exit', node)
      if (kduStack && kduStack.node === node) {
        callVisitor('onKduObjectExit', node)
        kduStack = kduStack.parent
      }
    }
    if (
      ...
      kduVisitor[
        ...
        if (kduStack && prop.parent === kduStack.node && prop.value === node) {
  ...
  getKduObjectType,
  /**
   * Get the Kdu component definition type from given node
   * Kdu.component('xxx', {}) || component('xxx', {})
   * @param {ObjectExpression} node Node to check
   * @returns {'component' | 'mixin' | 'extend' | 'createApp' | 'defineComponent' | null}
   */
  getKduComponentDefinitionType,
  /**
  ...
  /**
   * Check if current file is a Kdu instance (new Kdu) and call callback
   * @param {RuleContext} context The ESLint rule context object.
   * @param { (node: ObjectExpression, type: KduObjectType) => void } cb Callback function
   */
  executeOnKduInstance(context, cb) {
    return {
      /** @param {ObjectExpression} node */
  ...
  /**
   * Check if current file is a Kdu component and call callback
   * @param {RuleContext} context The ESLint rule context object.
   * @param { (node: ObjectExpression, type: KduObjectType) => void } cb Callback function
   */
  executeOnKduComponent(context, cb) {
    return {
      /** @param {ObjectExpression} node */
      'ObjectExpression:exit'(node) {
        const type = getKduObjectType(context, node)
      ...
      "CallExpression > MemberExpression > Identifier[name='component']": (
        ...
        if (callee.type === 'MemberExpression') {
          ...
          if (
            ...
            // calleeObject.name === 'Kdu' && // Any names can be used in Kdu.js 3.x. e.g. app.component()
  ...
  *iterateArrowFunctionExpression(node, groupName) {
  ...
  /**
   * Checks whether the given node is KElement.
   */
  isKElement,
  /**
  ...
  /**
   * Wraps composition API trace map in both 'kdu' and '@kdujs/composition-api' imports
   * @param {import('eslint-utils').TYPES.TraceMap} map
   */
  createCompositionApiTraceMap: (map) => ({
    kdu: map,
    '@kdujs/composition-api': map
  }),
...
/**
 * Checks whether the given node is KElement.
 * @param {KElement | KExpressionContainer | KText} node
 * @returns {node is KElement}
 */
function isKElement(node) {
  return node.type === 'KElement'
}
...
/**
 * Gets the KExpressionContainer of a given node.
 * @param {ASTNode} node - The node to get.
 * @return {KExpressionContainer|null}
 */
function getKExpressionContainer(node) {
  /** @type {ASTNode | null} */
  let n = node
  while (n && n.type !== 'KExpressionContainer') {
    n = n.parent
  }
  return n
}
 ...
 * @param {string} path
 */
function isKduFile(path) {
  return path.endsWith('.kdu') || path.endsWith('.jsx')
}
...
/**
 * Check whether the given node is a Kdu component based
 * on the filename and default export type
 * export default {} in .kdu || .jsx
 * @param {ESNode} node Node to check
 * @param {string} path File name with extension
 * @returns {boolean}
 */
function isKduComponentFile(node, path) {
  return (
    isKduFile(path) &&
 ...
 * @returns {'component' | 'mixin' | 'extend' | 'createApp' | 'defineComponent' | null}
 */
function getKduComponentDefinitionType(node) {
  ...
  if (parent.type === 'CallExpression') {
    ...
    if (callee.type === 'MemberExpression') {
      ...
      if (calleeObject.type === 'Identifier') {
        ...
        if (calleeObject.name === 'Kdu') {
          // for Kdu.js 2.x
          // Kdu.component('xxx', {}) || Kdu.mixin({}) || Kdu.extend('xxx', {})
          const maybeFullKduComponentForKdu2 =
            propName && isObjectArgument(parent)

          return maybeFullKduComponentForKdu2 &&
            (propName === 'component' ||
              propName === 'mixin' ||
              propName === 'extend')
            ? propName
        ...
        const maybeFullKduComponent = propName && isObjectArgument(parent)

        return maybeFullKduComponent &&
    ...
    if (callee.type === 'Identifier') {
      if (callee.name === 'component') {
        // for Kdu.js 2.x
        // component('xxx', {})
        const isDestructedKduComponent = isObjectArgument(parent)
        return isDestructedKduComponent ? 'component' : null
      }
      if (callee.name === 'createApp') {
        // for Kdu.js 3.x
        // createApp({})
        const isAppKduComponent = isObjectArgument(parent)
        return isAppKduComponent ? 'createApp' : null
      }
      if (callee.name === 'defineComponent') {
        // for Kdu.js 3.x
        // defineComponent({})
        const isDestructedKduComponent = isObjectArgument(parent)
        return isDestructedKduComponent ? 'defineComponent' : null
      }
    }
  }
...
/**
 * Check whether given node is new Kdu instance
 * new Kdu({})
 * @param {NewExpression} node Node to check
 * @returns {boolean}
 */
function isKduInstance(node) {
...
/**
 * If the given object is a Kdu component or instance, returns the Kdu definition type.
 * @param {RuleContext} context The ESLint rule context object.
 * @param {ObjectExpression} node Node to check
 * @returns { KduObjectType | null } The Kdu definition type.
 */
function getKduObjectType(context, node) {
  ...
  if (parent.type === 'ExportDefaultDeclaration') {
    ...
    if (
      isKduComponentFile(parent, filePath) &&
    ...
    // Kdu.component('xxx', {}) || component('xxx', {})
    if (
      getKduComponentDefinitionType(node) != null &&
...
function getComponentComments(context) {
  ...
  tokens = sourceCode
    .getAllComments()
    .filter((comment) => /@kdujs\/component/g.test(comment.value))
...
function getDirectives(node, name) {
  const attributes =
    node.type === 'KElement' ? node.startTag.attributes : node.attributes
  return attributes.filter(
    /**
     * @param {KAttribute | KDirective} node
     * @returns {node is KDirective}
     */
    (node) => {
