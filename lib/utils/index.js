...
module.exports = {
  ...
  defineTemplateBodyVisitor (context, templateBodyVisitor, scriptVisitor) {
    if (context.parserServices.defineTemplateBodyVisitor == null) {
      context.report({
        loc: { line: 1, column: 0 },
        message: 'Use the latest kdu-eslint-parser. See also https://github.com/kdujs/eslint-plugin-kdu#what-is-the-use-the-latest-kdu-eslint-parser-error'
      })
      return {}
    }
   ...
   */
  isRootElement (node) {
    assert(node && node.type === 'KElement')

    return (
      node.parent.type === 'KDocumentFragment' ||
      node.parent.parent.type === 'KDocumentFragment'
    )
  },
  ...
  hasAttributeValue (node) {
    assert(node && node.type === 'KAttribute')
    return (
      node.value != null &&
   ...
   * @returns {boolean}
   */
  isKduComponentFile (node, path) {
    const isKduFile = path.endsWith('.kdu') || path.endsWith('.jsx')
    return isKduFile &&
  ...
  /**
   * Check whether given node is Kdu component
  ...
  isKduComponent (node) {
    if (node.type === 'CallExpression') {
      ...
      if (callee.type === 'MemberExpression') {
        ...
        const isFullKduComponent = calleeObject.type === 'Identifier' &&
          calleeObject.name === 'Kdu' &&
          callee.property.type === 'Identifier' &&
          ['component', 'mixin', 'extend'].indexOf(callee.property.name) > -1 &&
          node.arguments.length >= 1 &&
          node.arguments.slice(-1)[0].type === 'ObjectExpression'

        return isFullKduComponent
      }

      if (callee.type === 'Identifier') {
        const isDestructedKduComponent = callee.name === 'component' &&
          node.arguments.length >= 1 &&
          node.arguments.slice(-1)[0].type === 'ObjectExpression'

        return isDestructedKduComponent
      }
    }
  ...
  executeOnKduInstance (context, cb) {
    ...
    return {
      'NewExpression:exit' (node) {
        // new Kdu({})
        if (!_this.isKduInstance(node)) return
        cb(node.arguments[0])
      }
    }
  },
  ...
  executeOnKduComponent (context, cb) {
    ...
    const _this = this
    const componentComments = sourceCode.getAllComments().filter(comment => /@kdujs\/component/g.test(comment.value))
    const foundNodes = []
    ...
    return {
      ...
      'ExportDefaultDeclaration:exit' (node) {
        // export default {} in .kdu || .jsx
        if (!_this.isKduComponentFile(node, filePath) || isDuplicateNode(node.declaration)) return
        cb(node.declaration)
      },
      'CallExpression:exit' (node) {
        // Kdu.component('xxx', {}) || component('xxx', {})
        if (!_this.isKduComponent(node) || isDuplicateNode(node.arguments.slice(-1)[0])) return
