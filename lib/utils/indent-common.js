...
function isNotEmptyTextNode (node) {
  return !(node.type === 'KText' && node.value.trim() === '')
}
...
module.exports.defineVisitor = function create (context, tokenStore, defaultOptions) {
  if (!context.getFilename().endsWith('.kdu')) return {}
  ...
  return processIgnores({
    ...
    KElement (node) {
      if (!PREFORMATTED_ELEMENT_NAMES.includes(node.name)) {
        const isTopLevel = node.parent.type !== 'KElement'
    ...
    "KElement[parent.type!='KElement']" (node) {
      processTopLevelNode(node, 0)
    },

    // Do validation.
    ":matches(Program, KElement[parent.type!='KElement']):exit" (node) {
      let comments = []
