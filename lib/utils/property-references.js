/**
 * @author NKDuy
 * @copyright 2021 NKDuy. All rights reserved.
...
function definePropertyReferenceExtractor(context) {
  ...
  function extractFromExpression(node, withInTemplate) {
    ...
    function extractWithoutCache() {
      ...
      if (parent.type === 'AssignmentExpression') {
        ...
        parent.type === 'KExpressionContainer' ||
    ...
    function maybeExternalUsed(parentTarget) {
      if (
        ...
        parentTarget.type === 'KExpressionContainer'
      ) {
        return true
      }
  ...
  /**
   * Extract the property references from KExpressionContainer.
   * @param {KExpressionContainer} node
   * @param {object} [options]
   * @param {boolean} [options.ignoreGlobals]
   * @returns {IPropertyReferences}
   */
  function extractFromKExpressionContainer(node, options) {
  ...
  return {
    ...
    extractFromKExpressionContainer,
